//! Build script — bake a `pickles_verifier::Verifier` blob into the ZisK
//! guest. Identical in spirit to `crates/o1-verifier/build.rs` (the SP1
//! guest): nothing here is SP1- or ZisK-specific, it's just a host-side
//! pre-bake of the verifier constants.
//!
//! Reads `VK_JSON` (the wrap `vk.serde.json` path), parses it via
...
//! to `OUT_DIR/verifier.bin`. The guest `include_bytes!`s it and
//! reinstantiates the verifier mostly zero-parse (pod-cast SRSes + postcard
//! wrap VK).
//!
